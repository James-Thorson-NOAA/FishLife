...
Plot_taxa_panel = function( lowerTaxa, upperTaxa=lowerTaxa, prob=0.95, params=names(Y_ij), xlim=log(c(0.01,2)), ylim=xlim, ticks=c(1,2,5), partial_match=FALSE, ... ){
  ...
  for( colI in 1:length(params)){
    ...
    lowerY = ParHat$beta_gj[lowerWhich,params[rowI]]
    lowerX = ParHat$beta_gj[lowerWhich,params[colI]]
    upperY = ParHat$beta_gj[upperWhich,params[rowI]]
    upperX = ParHat$beta_gj[upperWhich,params[colI]]
    if( rowI==colI ){
