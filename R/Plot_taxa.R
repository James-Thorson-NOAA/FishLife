...
Plot_taxa = function( Taxa, prob=0.95, params=matrix(c("K","M","Winfinity","Loo","tmax","tm","Lm","Temperature"),ncol=2,byrow=TRUE),
  ...
  for( rowI in 1:nrow(params)){
    ...
    for( uniqueI in 1:length(unique(Taxa)) ){
      ...
      for( aI in 1:2 ){
        Text = switch( params[rowI,aI], "Loo"="Asymptotic length (L_inf)", "K"="Relative growth rate (K)", "Winfinity"="Asymptotic mass (W_inf)", "tmax"="Maximum age (A_max)", "tm"="Age at maturity (A_mat)", "M"="Mortality rate (M)", "Lm"="Length at maturity (L_mat)", "Temperature"="Average temperature", "ln_var"="Conditional recruitment variance", "rho"="Recruitment autocorrelation (rho)", "ln_MASPS"="Maximum annual spawners per spawner", "ln_margsd"="SD of recruitment (Sigma_R)", "h"="Steepness (h)", "logitbound_h"="Steepness (h)", "ln_Fmsy_over_M"="Ratio of F_msy and M", "ln_Fmsy"="Fishing mortality rate at MSY", params[rowI,aI] )
