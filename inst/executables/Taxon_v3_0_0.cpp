...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_INTEGER( j_logM ); // Gives index j for ln_M in Y_ij
  ...
  // Transform betainput_gj to produce beta_gj
    // Required to deal with factor-valued traits, where sum-to-one constraint applies
  // First figure out which columns belong to a group
  vector<int> numlevels_j( group_j.size() );
  numlevels_j.setZero();
  for( int j=0; j<n_j; j++ ){
    numlevels_j(group_j(j)) += int(1);
  }
  ...
  for( int j1=0; j1<n_j; j1++ ){
    if( numlevels_j(j1) >= 2 ){
      for( int g=0; g<n_g; g++ ){
        // Get sum for that group in a given row
        sum_exp_plus_one = 1; // Start with exp(0)=1 for missing level
        for( int j2=0; j2<n_j; j2++ ){
          if( group_j(j2)==j1 ){
            beta_gj(g,j2) = exp(betainput_gj(g,j2));
            sum_exp_plus_one += beta_gj(g,j2);
          }
        }
        // Transform to sum to one in a given row
        for( int j2=0; j2<n_j; j2++ ){
          if( group_j(j2)==j1 ){
            beta_gj(g,j2) = beta_gj(g,j2) / sum_exp_plus_one;
          }
        }
        // log-transform as experiment
        for( int j2=0; j2<n_j; j2++ ){
          if( group_j(j2)==j1 ){
  ...
  for( int StockI=0; StockI<Nstock; StockI++ ){
    ...
    if( (Options_vec(6)==1) | (Options_vec(6)==2) ){  // Logit-space
      ...
      if( Options_vec(6)==2 ){
        jnll_comp(11) += (1-PredTF_stock(StockI)) * 2 * exp(Ycomplete_ij(i_stock(StockI),j_SR(1))) / pow(1.0+exp(Ycomplete_ij(i_stock(StockI),j_SR(1))),2);   //# reciprocal of derivative of logit-function = derivative of logistic function
      }
    }
    // Extract MASPS
    if( Options_vec(5)==1 ){
    ...
    if( j_SR.size()==3 ){
      ...
      if( Options(2)>0 ) jnll_comp(6) -= (1-PredTF_stock(StockI)) * dnorm( log(b_stock(StockI)), Type(0.0), Options(2), true );
    }
    if( j_SR.size()==4 ){
      if( Options_vec(3)==0 ){
  ...
  for( int ObsI=0; ObsI<Nobs; ObsI++ ){
    ...
    if( AR_Index(ObsI) == -1 ){
      ...
      jnll_obs(ObsI) = -1 * (1-PredTF_stock(Obs2Stock(ObsI))) * dnorm( ln_R_obs(ObsI), mu_obs(ObsI), SD_stock(Obs2Stock(ObsI)), true );
    }
    if( AR_Index(ObsI) == 0 ){
    ...
    if( AR_Index(ObsI) == 1 ){
      mu_obs(ObsI) = ln_R_obs_hat(ObsI);
      jnll_obs(ObsI) = -1 * (1-PredTF_stock(Obs2Stock(ObsI))) * dnorm( ln_R_obs(ObsI), mu_obs(ObsI), SD_stock(Obs2Stock(ObsI))/pow(1-pow(ro_stock(Obs2Stock(ObsI)),2),0.5), true );
    }
  }
