...
template<class Type>
matrix<Type> cov_matrix( vector<Type> L_val, vector<Type> logmult_col, Type min_var, int n_rows, int n_cols ){
...
Type objective_function<Type>::operator() ()
{
  ...
  Cov_jj = cov_matrix(L_z, L_logmult_col, Options(1), n_j, Options_vec(1));
  jnll_comp(8) = -1 * sum(dnorm( L_logmult_col, Type(0.0), Type(1.0), true ));

  // Observation covariance
  matrix<Type> obsCov_jj( n_j, n_j );
  obsCov_jj = cov_matrix(obsL_z, obsL_logmult_col, Options(0), n_j, Options_vec(0));
