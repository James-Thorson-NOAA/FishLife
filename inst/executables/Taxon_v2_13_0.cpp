...
Type objective_function<Type>::operator() ()
{
  ...
  for( int StockI=0; StockI<Nstock; StockI++ ){
    ...
    if( (Options_vec(6)==1) | (Options_vec(6)==2) ){  // Logit-space
      ...
      if( Options_vec(6)==2 ){
        jnll_comp(11) += 2 * exp(Ycomplete_ij(i_stock(StockI),j_SR(1))) / pow(1.0+exp(Ycomplete_ij(i_stock(StockI),j_SR(1))),2);   //# reciprocal of derivative of logit-function = derivative of logistic function
      }
    }
    // Extract MASPS
    if( Options_vec(5)==1 ){
